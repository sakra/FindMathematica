...
set (Mathematica_CMAKE_MODULE_VERSION "1.0.0")
...
function (_add_registry_search_path _registryKey _outSearchPaths)
	...
	if (${_qualified})
		get_filename_component (
			_path "[${_registryKey};ExecutablePath]" PATH)
		if (EXISTS "${_path}")
			_to_cmake_path("${_path}" _path)
			get_filename_component (
...
# internal macro to determine Mathematica installation paths from Mac OS X launch services database
...
macro(_get_search_paths _outSearchPaths)
	...
	if (CMAKE_HOST_WIN32)
		...
		foreach (_envVar IN LISTS _WindowsProgramFilesEnvVars)
			if (CYGWIN)
				# Cygwin requires environment variables converted to upper case
				string (TOUPPER ${_envVar} _envVar)
			endif()
...
macro(_systemNameToSystemID _systemName _systemProcessor _outSystemIDs)
	if (${_systemName} MATCHES "Windows")
		if (${_systemProcessor} MATCHES "AMD64")
			set (${_outSystemIDs} "Windows-x86-64")
		else()
			# default to 32-bit Windows
			set (${_outSystemIDs} "Windows")
		endif()
	elseif (${_systemName} MATCHES "CYGWIN")
		set (${_outSystemIDs} "Windows")
	elseif (${_systemName} MATCHES "Darwin")
		if (${_systemProcessor} MATCHES "i386")
			set (${_outSystemIDs} "MacOSX-x86")
		elseif (${_systemProcessor} MATCHES "x86_64")
			set (${_outSystemIDs} "MacOSX-x86-64")
		elseif (${_systemProcessor} MATCHES "ppc64")
	...
	elseif (${_systemName} MATCHES "Linux")
		...
		elseif (${_systemProcessor} MATCHES "ia64")
			set (${_outSystemIDs} "Linux-IA64")
		endif()
	elseif (${_systemName} MATCHES "SunOS")
	...
	elseif (${_systemName} MATCHES "AIX")
		set (${_outSystemIDs} "AIX-Power64")
	elseif (${_systemName} MATCHES "HP-UX")
		set (${_outSystemIDs} "HPUX-PA64")
	elseif (${_systemName} MATCHES "IRIX")
...
macro(_get_system_IDs _outSystemIDs)
	...
	if (NOT DEFINED ${_outSystemIDs})
		message (WARNING "Unknown system ${CMAKE_SYSTEM_NAME}-${CMAKE_SYSTEM_PROCESSOR}")
...
macro(_get_host_mathlink_flavor _outMathLinkFlavor)
	if (CMAKE_HOST_WIN32)
		if (CYGWIN)
			set (${_outMathLinkFlavor} "cygwin")
		elseif ("$ENV{PROCESSOR_ARCHITECTURE}" STREQUAL "AMD64" OR
				"$ENV{PROCESSOR_ARCHITEW6432}" STREQUAL "AMD64")
			# host is native 64-bit Windows
			set (${_outMathLinkFlavor} "mldev64")
		else()
...
macro(_get_host_library_search_path_envvars _outVariableNames)
	...
	elseif (CMAKE_HOST_UNIX)
		...
		elseif (${CMAKE_HOST_SYSTEM_NAME} MATCHES "IRIX")
